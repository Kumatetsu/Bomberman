...
void *draw_map_model(void *arg)
{
  ...
  for (j = J_BEGIN; j < J_BEGIN + 13; j++)
      ...
      for (i = I_BEGIN; i < I_BEGIN + 15; i++)
	{
	  ...
	  // we put the square in array_map beginning at a == 1 to let enter le first line
...
int		draw_timer(void *arg)
{
  ...
  SDL_Rect	dest_rect_timer = {
    (WINDOW_W / 2) - ((timer_panel_rect.w / 2) * 5),
    (dest_rect_score.h / 2) - ((timer_panel_rect.h / 2) * 5),
...
void			draw_destroyable_model(void *arg)
{
  ...
  for (i = 0; i < INLINE_MATRIX; i++)
	...
	} else if (destroyable.explosion_stage > 0) {

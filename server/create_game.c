...
void    specify_player_info(int i, t_game_info *game_info)
{
   ...
   * ordonnée 13 blocks
   * abscice  15 blocks
   * On met le joueur au milieu d'une box (pour exemple le joueur 1 est à 48 + 24 car case numéro 2)
  **/
  switch (i)
    {
    case 0:
      game_info->players[i].x_pos = 72;
      game_info->players[i].y_pos = 72;
      game_info->players[i].current_dir = BOMBER_L;
      break;
    case 1:
      game_info->players[i].x_pos = 648;
      game_info->players[i].y_pos = 72;
      game_info->players[i].current_dir = BOMBER_D;
      break;
    case 2:
      game_info->players[i].x_pos = 72;
      game_info->players[i].y_pos = 552;
